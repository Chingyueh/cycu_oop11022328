...
class taipei_route_info:
    ...
    def fetch_bus_status(self) -> dict:
        """
        Fetches the bus status from the main website.

        Returns:
            dict: A dictionary containing the bus stop numbers and their statuses (e.g., arriving or not).
        """
        ...
        # 模擬解析進站中的公車資料（需根據實際網站內容調整）
        pattern = re.compile(r'<li.*?data-stop-number="(\d+)".*?class="arriving">', re.DOTALL)
        matches = pattern.findall(content)
        return {int(stop_number): True for stop_number in matches}

    def plot_station_on_map(self, stop_number: int, bus_status: dict):
        """
        Plots a specific bus stop on a map using its stop number.

        Args:
            stop_number (int): The stop number to plot on the map.
            bus_status (dict): A dictionary containing the bus stop numbers and their statuses.
        """
        # 確保 dataframe 已初始化
        ...
        # 如果該站有公車進站，添加公車圖標
        if stop_number in bus_status and bus_status[stop_number]:
            bus_icon = CustomIcon(
                icon_image=r"C:/Users/User/Desktop/cycu_oop11022328/20250429/bus.jpg",
                icon_size=(50, 50)  # 圖片大小
            )
            folium.Marker(
                location=map_center,
                popup="Bus is arriving!",
                icon=bus_icon
            ).add_to(m)
...
if __name__ == "__main__":
    # 測試程式
    route_id = input("Enter the bus route ID: ")  # 使用者輸入公車幹線代碼
    route_info = taipei_route_info(route_id=route_id, direction="go")
    ...
    # 從網站抓取進站中的公車資訊
    bus_status = route_info.fetch_bus_status()

    # 輸入站牌號碼
    stop_number = int(input("Enter the stop number to plot on the map: "))

    # 繪製地圖
    route_info.plot_station_on_map(stop_number, bus_status)