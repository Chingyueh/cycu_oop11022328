...
class taipei_route_info:
    ...
    def plot_station_on_map(self, stop_number: int, bus_arriving: bool = False):
        """
        Plots a specific bus stop on a map using its stop number.
        Optionally adds a bus icon if the bus is arriving.
...
if __name__ == "__main__":
    ...
    for route_id in bus_list:
        try:
            ...
            bus_arriving = input("Is the bus arriving at this stop? (yes/no): ").strip().lower() == "yes"
            route_info.plot_station_on_map(stop_number, bus_arriving)
