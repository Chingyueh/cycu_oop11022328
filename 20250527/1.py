...
class taipei_route_info:
    ...
    def download_html(self):
        ...
        嘗試下載缺少的 HTML 檔案並儲存。
        """
        url = f'https://ebus.gov.taipei/Route/StopsOfRoute?routeid={self.route_id}'
        ...
        if response.status_code == 200:
            ...
            print(f"下載 HTML 檔案失敗，路線 {self.route_id}，狀態碼：{response.status_code}")
...
if __name__ == "__main__":
    ...
    for _, row in route_df.iterrows():
        try:
            ...
            df = pd.DataFrame(dict_wkt.items(), columns=['WKT欄位', 'WKT字串'])
            df['路線ID'] = route_info.route_id
            df['路線名稱'] = row["route_name"]

            # 轉換為 GeoDataFrame
            df['幾何'] = gpd.GeoSeries.from_wkt(df['WKT字串'])
            gdf = gpd.GeoDataFrame(df, geometry='幾何')

            # 設定 CRS 為 WGS84
            gdf.crs = 'EPSG:4326'
            gdf.set_geometry('幾何', inplace=True)
