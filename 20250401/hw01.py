...
if response.status_code == 200:
    ...
    with open("bus1.html", "r", encoding="utf-8") as file:
        content = file.read()
        decoded_content = html.unescape(content).replace("\xa0", " ")  # 過濾特殊字符
        print(decoded_content.encode("utf-8", errors="replace").decode("utf-8"))  # 顯示解碼後的內容
    ...
    for table in tables:
        ...
        for tr in table.find_all("tr", class_=["ttego1", "ttego2"]):
            ...
            td = tr.find("td")
            if td:
                stop_name = html.unescape(td.text.strip()).replace("\xa0", " ")  # 過濾特殊字符
